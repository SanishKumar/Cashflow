...
export function LedgerPage() {
  ...
  const allTransactions = (allGroupsTx ?? [])
    .flatMap((gwt) =>
      gwt.transactions.map((tx) => ({ ...tx, groupName: gwt.group.name, groupId: gwt.group.id }))
    )
  ...
  return (
    <div className="h-full flex flex-col">
      {/* Header */}
      <header className="h-14 border-b border-outline-variant/30 flex items-center px-6 justify-between bg-surface-container/50 shrink-0">
        <div className="flex items-center gap-3">
          <span className="material-symbols-outlined text-on-surface-variant text-[20px]">receipt_long</span>
          <h2 className="text-[15px] font-semibold text-on-surface">Global Ledger</h2>
        ...
        {isLoading ? (
          ...
          <div className="rounded-lg border border-outline-variant/30 overflow-hidden mt-2">
            ...
            <div className="grid grid-cols-12 gap-3 px-4 py-2.5 bg-surface-dim border-b border-outline-variant/30">
              ...
              <div className="col-span-2 text-label text-[10px]">Paid By</div>
              <div className="col-span-2 text-label text-[10px] text-right">Amount</div>
              <div className="col-span-1 text-label text-[10px] text-right">Split</div>
            </div>
            {/* Rows */}
            {allTransactions.map((tx, i) => (
              ...
              >
                ...
                <div className="col-span-3 text-[13px] font-medium text-on-surface truncate">{tx.description}</div>
